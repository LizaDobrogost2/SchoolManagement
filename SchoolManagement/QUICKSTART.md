...
### Step 3: Add Students to Class (RESTful PATCH Approach) ? RECOMMENDED

**The RESTful way** - Update the student resource directly:
...
curl -k -X PATCH https://localhost:5001/api/students/S002 \
  -H "Content-Type: application/json" \
  -d '{"schoolClassId":1}'

# Assign Student 3 to Class 2
curl -k -X PATCH https://localhost:5001/api/students/S003 \
  -H "Content-Type: application/json" \
  -d '{"schoolClassId":2}'
```
...
```bash
# The old way (deprecated but still works for backward compatibility)
...
### Step 7: Update a Student (Full Update with PUT)
...
### Step 7b: Partial Update a Student (RESTful PATCH) ? NEW
...
# Or partial update with PATCH ? NEW
curl -k -X PATCH https://localhost:5001/api/classes/1 \
  -H "Content-Type: application/json" \
  -d '{"name":"Class 5A Elite"}'
```

### Step 9: Remove Student from Class (RESTful PATCH) ? RECOMMENDED

```bash
# Set schoolClassId to null to unassign
curl -k -X PATCH https://localhost:5001/api/students/S002 \
  -H "Content-Type: application/json" \
  -d '{"schoolClassId":null}'

# Or use 0 as alternative to null
curl -k -X PATCH https://localhost:5001/api/students/S002 \
  -H "Content-Type: application/json" \
  -d '{"schoolClassId":0}'
```
...
### Step 12: Move Student Between Classes (RESTful) ? NEW

```bash
# Move student from Class 1 to Class 2
curl -k -X PATCH https://localhost:5001/api/students/S001 \
  -H "Content-Type: application/json" \
  -d '{"schoolClassId":2}'
```

This automatically removes the student from Class 1 and assigns them to Class 2.
...
| PATCH | `/api/students/{id}` | Yes | Partial update student |
...
| PATCH | `/api/classes/{id}` | Yes | Partial update class |
